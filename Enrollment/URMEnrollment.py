...
df = pd.read_csv(r'Enrollment\enrollmentByEthnicity_2004-2020.csv')             # Read necessary csv
...
for string in years:                                                 
    ...
    yearsInList2.insert(count3, string)
    count+=1
...
while count2 < go:
    try:
        years2.insert(count2, yearsInList2[count2][1])
        count2+=1
    except:
        years2.insert(count2, yearsInList2[count2][0])
        count2+=1


#print(years2)

distictYears = pd.unique(years2) 
...
count = 0
while count <= 10:
    answ = df.iloc[:, nameee] + df.iloc[:, hisp]+ df.iloc[:, ai]+  df.iloc[:, two]+ df.iloc[:, nw]
    df2[distictYears[count]] = answ
...
df2.to_csv(r'Enrollment\URMenrollment.csv')
print(df2)
