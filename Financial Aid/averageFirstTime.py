...
df = pd.read_csv(r'Financial Aid\InstAidtoFTFTCohort_2002_2018.csv')             # Read necessary csv
...
sub = "Average amount of institutional grant aid awarded to full-time"
for string in cols:     
    print(string)                                                      # for every item in list find the ones with numbers and add it to colsWithNums 
    if sub in string:
        print(string)
        averageCols.insert(count, string)
        count+=1
...
for string in averageCols:                                                     # for x in list of columns with numbers split according to (
    string = string.split("(")
...
df2.to_csv(r'Financial Aid\averageFirstTime.csv')
print(df2)
