...
df = pd.read_csv(r'Faculty\T-TTFacHeadcount_2012_2020.csv')             # Read necessary csv
...
distictYears = pd.unique(numsWithF) 
print(distictYears)
...
for x in distictYears:
    df2[x] = df.iloc[:, tenure] + df.iloc[:, tenure1]
    tenure1 +=2
    tenure +=2
...
df2.to_csv(r'Faculty\headcountTenure.csv')
print(df2)
